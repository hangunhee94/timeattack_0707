...
class ApplyView(APIView):
    ...
    def post(self, request):
        ...
        job_post_qs = JobPost.objects.filter(id=job_post)
        if not job_post_qs.exists():
            return Response({"message": "유효하지 않은 공고입니다."}, status=status.HTTP_400_BAD_REQUEST)

        Apply_serializer = UserApplySerializer(data=request.data)
        if Apply_serializer.is_valid():
            Apply_serializer.save()
            return Response({"message": "지원 완료!"})
        else:
            print(Apply_serializer.errors)
            return Response({"message": "지원 실패!"})
